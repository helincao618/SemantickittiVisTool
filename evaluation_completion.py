...
def load_pred_volume(filename):
  labels = np.fromfile(filename, dtype=np.uint16).reshape(256,256,64)[:128,:128,:32]
  labels_up = np.zeros([256,256,32], dtype=np.uint16)
  labels_up[::2,::2,:] = labels
  labels_up[1::2,::2,:] = labels
  labels_up[::2,1::2,:] = labels
  labels_up[1::2,1::2,:] = labels
  return labels_up.reshape(-1)
...
if __name__ == "__main__":
  ...
  for i, f in tqdm(enumerate(evaluation_pairs)):
