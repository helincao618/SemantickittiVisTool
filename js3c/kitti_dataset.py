...
config_file = os.path.join('JS3C-Net/opt/semantic-kitti.yaml')
...
class get_dataset(Dataset):
    def __init__(self, config, split="train", augment=False):
        ...
        self.files = {}#dict of type of file, each item store the path to corresponding file
        self.filenames = []#（sequence，index）
        self.split = split
        ...
        for sequence in SPLIT_SEQUENCES[split]:
            ...
            for ext in SPLIT_FILES[split]:
                ...
                self.files[EXT_TO_NAME[ext]].extend(comletion_data) #add file with different type to dict
